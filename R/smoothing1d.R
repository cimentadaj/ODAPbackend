...
#' @title `check_smooth`
#' @description Creates a plot of original data as points and smooth result as line for each `.id`.
...
#' @return A figure of original data as points and smooth result as line for each `.id`
...
#' xout <- 1950:2018 + .5
#' 
#' data_out <- smooth1d(data_in, method = "gam-ps", xout = xout)
#' 
#' check_smooth(data_in, data_out)
#' 

check_smooth <- function(data_in,
                         data_out) { 
