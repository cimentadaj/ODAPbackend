...
#' remain unchanged.
#'
#' @param location Optional. Country or region name/ISO code. Used if inputs are to be automatically downloaded from WPP.
#' @param refDate Numeric or `Date`. Reference date of the population count (e.g., census year and month, converted internally to decimal date).
#' @param Age Integer vector. Lower bounds of single-year ages (if `NULL`, inferred from input names or length of `Females_single`).
#' @param Females_single Numeric vector. Female population counts by single year of age at `refDate`.
#' @param Males_single Optional numeric vector. Male population counts by single year of age at `refDate`.
#' @param nLxFemale,nLxMale Life table person-years lived by age and sex. Can be provided directly or downloaded if `location` is supplied.
#' @param nLxDatesIn Numeric vector of input dates corresponding to `nLxFemale` and `nLxMale`. Defaults to `refDate - c(0.5, 9.5)`.
#' @param AsfrMat Matrix or data.frame of age-specific fertility rates (ASFR),with ages 15–49 in rows and dates in columns.
#' @param AsfrDatesIn Numeric vector of input dates for `AsfrMat`. Defaults to `refDate - c(0.5, 9.5)`.
#' @param SRB Optional numeric vector of sex ratios at birth (males per female). If `NULL`, values are downloaded when `location` is provided.
#' @param SRBDatesIn Numeric vector of input dates for `SRB`. Defaults to the three anchor dates `refDate - c(0.5, 2.5, 7.5)`.
