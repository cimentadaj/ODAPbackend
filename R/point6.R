...
basepop_single <- function(location = NULL,
  ...
  # S0 is Lx(1) / Lx(0)
  # S2 is LX2 / Lx1
  # for forward survival of births (maybe not product, we want the opposite of Sx)
  # we start on the rights female single year and we get Sx for the previous year and the inverse of that
  # we multiply and it shifts down in age and we go to age 59 (cumprod) 
  # reverse survival for filling female exposures multipluied by downloaded Asfr = births
  # then take colsums and it is the birth factor
  # then take births and survive them back year t (different triangle)
  # 
  
  # single loop over ages or cohorts
  ...
  # remake with single loop not two, where the Bt is vector of length 10, 
  # 470 asfr 2 columns exposure 10 7?
  # then also adjust PF and nLxf
  # SRB is a vectorand then if d is indexing it it pulls from the vector that does not have these names
  # and the return is not correct
  # we want x age groups or cohor years 
  # nLx we need to survive them from birth to position in census
  # then the dates out are ref dates 
  # so we can just take those nLx values since they are on singe year grid
  # use population data to obtaion exposures
  # 465 we can get the exposure from wpp and not the user
  # get cohort Sx for reverse surviving in demotools
